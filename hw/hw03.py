...
def pascal(row, column):
    ...
    list = [1]
    while row >= len(list):
        list = [1] + [list[i] + list[i + 1] for i in range(0, len(list) - 1)] + [1]
    if column > len(list):
        return 0
    return list[column]
...
def move_stack(n, start, end):
    ...
    if n == 1:
        ...
        other = 6 - start - end  # 另一个柱子
        move_stack(n - 1, start, other)  # n - 1 移动到辅助柱子
        print_move(start, end)
        move_stack(n - 1, other, end)  # n - 1 个从辅助柱子移动到目标柱子
